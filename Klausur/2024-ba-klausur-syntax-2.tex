...
%# Aufgabe mit V2/Scrambling/Muster ist doof, weil man nur 5 oder 0 Punkte kriegen kann.
...
\item Für die Bearbeitung der Klausur stehen 30 Minuten zur Verfügung.
...
\begin{enumerate}
\item Welche der angeführten Sprachen ist Isländisch? (5)
\begin{itemize}
...
\item[$\bigcirc$] 对于一门发展中的、涉及内容广泛的学科而言这是正常的，但长期下去，又会对学科的发
展产生不利影响。 
\item[$\bigcirc$] keine.
\end{itemize}

% \item Wenn einen Aussagesatz mit der Abfolge XP V YP V aus einer beliebigen germanischen Sprache vorgelegt
%   bekommen, was können Sie dann über die typologische Einordnung sagen? (5)
% \begin{itemize}
...
% \item[$\bigcirc$] Es handelt sich um eine VSO-Sprache.
% \item[$\bigcirc$] Man kann die Sprache nicht einordnen.
% \end{itemize}

\item Welche Kasus kommen im Deutschen als lexikalische Kasus vor? Mehrfachnennungen möglich. [5]
\begin{itemize}
\item[$\bigcirc$] der Nominativ
\item[$\bigcirc$] der Genitiv
\item[$\bigcirc$] der Dativ
\item[$\bigcirc$] der Passiv
\item[$\bigcirc$] der Akkusativ
\end{itemize}
...
% \item Welches Phänomen bzw.\ welche Phänomene tritt/treten nur bei den germanischen OV-Sprachen und nicht bei den anderen
%   germanischen Sprachen auf? (5 Punkte)
% % 
% % Scrambling, Verbalkomplexbildung
% \begin{itemize}
% \item[$\bigcirc$] V2
% \item[$\bigcirc$] Scrambling
% \item[$\bigcirc$] Verbalkomplexbildung
% \item[$\bigcirc$] expletive Subjekte
% \item[$\bigcirc$] keines der vier genannten
% \end{itemize}


\item Analysieren Sie die folgenden Sätze (Angabe der Baumstruktur mit Kategorien und
  Valenzmerkmalen). NPen und PPen können Sie abkürzen. (30 Punkte)
\eal
\ex dass ihn niemand mag
%5
\ex that nobody likes him
%5
\ex Er mag ihn.
%10
\ex 
\gll Han elsker ham.\\
     er  mag    ihn\\\jambox{(Danish)}
%10
\zl
% \item Erklären Sie die Analyse des Passivs im Deutschen im in der Vorlesung besprochenen
%   theoretischen Rahmen. (15 Punkte)
\end{enumerate}
