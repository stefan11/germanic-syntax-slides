...
\item The equivalent in German would be:
	...
	Mich dünkt der Mann langweilig. \\
		I.\ACC{} thinks the.\NOM{} movie boring\\
...
\frame{
\frametitle{Quircky  Subject Passiv: Controll}
...
only need one lexicon entry for the verb  \emph{lesen} `to read' in (\mex{1}):
	...
	\eal
		\ex \gll \blau{Er} möchte das Buch lesen.\\
...
\pause
	\item Examples of structural cases are:
		...
		\eal
		\ex 
			\gll \blau{Der Installateur} kommt. \\
					the.\nom{} plumber      comes\\
					\glt `The plumber comes.			
		
		\pause
		
		\ex \gll Der Mann läßt \blau{den Installateur} kommen. \\
					the man  lets the.\acc{} plumber      come\\
					\glt `The man lets the plumber come.'
			
		\pause
		
		\ex \gll das Kommen \blau{des Installateurs} \\
...
in (\mex{1}) the case of the object of \emph{schlagen} `to defeat':
		...
		\eal
		\ex \gll Judit schlägt \blau{den Weltmeister}. \\
					Judit defeats the.\acc{} world.champion\\
					\glt `Judit defeats the world champion.'
					
		\ex \gll \blau{Der Weltmeister} wird geschlagen.\\
...
The case of a genitive object does not change with passivization.
		...
		\eal
			...
			\ex[*]{
				\gll \blau{Die Opfer} wird/werden gedacht. \\
...
\pause
\item Haiders \citeyearpar[20]{Haider86} Beispiele sind dann sofort erklärt. Analoge Beispiele:
\eal
\ex[]{\iw{streicheln}
Er       streichelt \blau{den Hund}.
}
\ex[]{
\blau{Der Hund} wird gestreichelt.
}
\ex[]{
...
\ex[]{
\blau{Den Kindern} wird geholfen.
}
\ex[]{
...
Vorhersage: Dativpassiv ist mit Verben mit lexikalischem Dativ nicht möglich.
...
\frame{
\frametitle{Ergebnis der Lexikonregelanwendung für das Dänische}
...
\subsection{Mapping auf \comps in OV-Sprachen}

\frame{
\frametitle{Mapping auf \comps in OV-Sprachen}

Alle Elemente von \argst werden auf \comps gemappt:
...
\frame[shrink=10]{
\frametitle{Alle Argumente des Partizips können vorangestellt werden}

\savespace
\smallexamples

Komplemente können vorangestellt werden:
\eal
\ex  Den        Wählern Märchen erzählen sollte man besser nicht.\\
\ex  Den        Wählern erzählen sollte man solche Märchen     nicht.\\
\ex  Märchen     erzählen sollte man den        Wählern nicht.\\
\zl
\pause
Das Subjekt aber normalerweise nicht:
\eal
\ex[*]{
...
PVP siehe: \citew{Mueller96a}, \citew[Chapter~2.2.2]{Mueller2002b} oder \citew[Chapter~15.2]{MuellerLehrbuch4}.
...
\frame[shrink=15]{
\frametitle{Analyse des Fernpassivs}
...
\begin{itemize}
\item \emph{versuchen} bettet Infinitiv mit \emph{zu} ein.
\pause
\item \emph{versuchen} zieht Argumente von \emph{reparieren} an: \argstw \sliste{ NP[\str]$_i$, NP[\str]$_j$, V[\type{inf}] }
\pause
\item Passiv-LR unterdrückt erstes Argument: \emph{versucht} hat
\argstw \sliste{ NP[\str]$_j$, V[\type{inf}] } 
\pause
\item \emph{zu reparieren versucht}: \argstw \sliste{ NP[\str]$_j$ } und
